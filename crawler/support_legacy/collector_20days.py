...
DEFAULT_OUTPUT_CSV = feature_csv_path("policy_updates_features_20days_revision.csv")
DEFAULT_INTERVAL_SEC = 24 * 60 * 60 # 하루 1회 실행 주기 (초 단위)
US_EASTERN_TZ = ZoneInfo("America/New_York")    # 미국 동부시간 기준
KEYWORD_CONFIG_DIR = Path("C:/Users/Lee/data-ml/crawler/collectors/keywords")  # American Presidential Project 관련 키워드 JSON 파일이 있는 디렉토리
...
def _collect_ucsb_records(
    ...
    records: list[dict[str, Any]] = []
    print(f"[UCSB] Starting crawl for {target_date.isoformat()} with doc_types={selected_doc_types}")
...
def run_monitor(
    ...
    while True:
        ...
        target_date = datetime.strptime("2026-08-11", "%Y-%m-%d").date()
        cycle += 1
...
def main() -> None:
    ...
    parser.add_argument("--max-cycles", type=int, default=1, help="모니터링 주기 반복 횟수 제한; None이면 무한 반복")
